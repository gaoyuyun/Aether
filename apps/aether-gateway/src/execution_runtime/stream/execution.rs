...
mod tests {
    ...
    use super::{
        ...
        client_format_allows_proxy_generated_sse_control_blocks, current_request_candidate_unix_ms,
    ...
    async fn stream_attempt_watchdog_timeout_is_not_recorded_as_client_cancellation() {
        ...
        progress
            .scope(async move {
                ...
                let snapshot = crate::request_candidate_runtime::snapshot_local_request_candidate_status(
                    &plan, Some(&context),
                );
                let mut guard = crate::execution_runtime::attempt_cancellation::AttemptCancellationGuard::disarmed(
                    &state, "downstream_disconnect", "client disconnected before stream response was ready",
                );
                guard.arm(&plan, Some(&context), snapshot.as_ref(), current_request_candidate_unix_ms(), Instant::now());
                progress_for_scope.mark_abandoned();
                drop(guard);
            })
            .await;
