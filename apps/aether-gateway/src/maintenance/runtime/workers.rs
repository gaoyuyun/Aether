...
use super::{
    ...
    perform_oauth_token_refresh_once, perform_provider_balance_monitor_once,
    ...
    POOL_MONITOR_INTERVAL, PROVIDER_BALANCE_MONITOR_INTERVAL, PROVIDER_CHECKIN_DEFAULT_TIME,
...
pub(crate) fn spawn_provider_balance_monitor_worker(
    state: AppState,
) -> Option<tokio::task::JoinHandle<()>> {
    if !state.has_provider_catalog_data_reader() {
        return None;
    ...
    Some(crate::task_runtime::spawn_singleton_worker(
        ...
        |state| async move {
            let mut interval = tokio::time::interval(PROVIDER_BALANCE_MONITOR_INTERVAL);
            interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
            interval.tick().await;
            let mut deferred_since = None;
            loop {
                ...
                if should_defer_for_database_pressure(
                    &state.data,
                    "provider_balance_monitor",
                    &mut deferred_since,
                ) {
                    continue;
                }
                if let Err(err) = perform_provider_balance_monitor_once(&state).await {
                    log_maintenance_worker_failure("provider_balance_monitor", "tick", &err);
                }
            }
        },
    ))
