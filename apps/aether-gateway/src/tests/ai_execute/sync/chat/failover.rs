...
async fn gateway_surfaces_local_execution_runtime_miss_reason_when_all_openai_chat_candidates_are_skipped_impl(
    ...
    let stored_candidates = request_candidate_repository
        ...
        .await
        .expect("request candidate trace should read");
    // Separate execution paths no longer overwrite the same skipped slot.
    assert_eq!(stored_candidates.len(), 2);
...
async fn gateway_retries_next_local_openai_chat_sync_candidate_after_auth_failure_impl() {
    ...
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        vec![
            ...
            sample_provider_catalog_provider("provider-openai-local-primary", "openai", Some(1)),
...
large_stack_async_test!(
    gateway_honours_provider_same_key_retry_overrides_on_every_candidate,
    gateway_honours_provider_same_key_retry_overrides_on_every_candidate_impl
);

async fn gateway_honours_provider_same_key_retry_overrides_on_every_candidate_impl() {
    #[derive(Debug, Clone)]
    struct SeenExecutionRuntimeSyncRequest {
        trace_id: String,
        url: String,
        ...
        any(move |request: Request| {
            ...
            async move {
                ...
                *hits += 1;
                ...
                // Attempt 1 is the primary's only try (no retry by default),
                // attempt 2 is the backup's first try and attempt 3 is the
                // backup's provider-level retry.
                if attempt <= 2 {
                    return Json(json!({
                        "request_id": "trace-openai-chat-provider-retry-override-123",
                        "status_code": 401,
                        "headers": {
                            "content-type": "application/json"
                        },
                ...
                Json(json!({
                    ...
                    "body": {
                        "json_body": {
                            "id": "chatcmpl-provider-retry-override-123",
                            "object": "chat.completion",
                            "model": "gpt-5-upstream-backup",
                            "choices": [{
                                "message": {"role": "assistant", "content": "done"}
    ...
    let auth_repository = Arc::new(InMemoryAuthApiKeySnapshotRepository::seed(vec![(
        Some(hash_api_key("sk-client-openai-provider-retry-override")),
        sample_auth_snapshot(
            "api-key-openai-provider-retry-override-1",
            "user-openai-provider-retry-override-1",
        ),
    )]));
    let candidate_selection_repository =
        Arc::new(InMemoryMinimalCandidateSelectionReadRepository::seed(vec![
    ...
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        vec![
            // The primary provider inherits the routing policy default of no
            // same-key retry; the backup provider allows one retry on each of
            // its keys.
            sample_provider_catalog_provider("provider-openai-local-primary", "openai", None),
            sample_provider_catalog_provider("provider-openai-local-backup", "openai", Some(1)),
        ],
        vec![
            sample_provider_catalog_endpoint(
                "endpoint-openai-local-primary",
    ...
    let response = reqwest::Client::new()
        ...
        .header(
            http::header::AUTHORIZATION,
            "Bearer sk-client-openai-provider-retry-override",
        )
        .header(
            TRACE_ID_HEADER,
            "trace-openai-chat-provider-retry-override-123",
        )
        .body("{\"model\":\"gpt-5\",\"messages\":[]}")
        .send()
        .await
    ...
    let seen_execution_runtime_requests = seen_execution_runtime
        ...
        .expect("mutex should lock")
        .clone();
    // Without a provider override the primary fails over immediately after
    // its first failure. The backup provider overrides to one retry, so it is
    // retried once on the same key although it is not the first-ranked
    // candidate.
    assert_eq!(seen_execution_runtime_requests.len(), 3);
    assert_eq!(
    ...
    for backup_request in &seen_execution_runtime_requests[1..] {
        assert_eq!(
    ...
    let stored_candidates = request_candidate_repository
        .list_by_request_id("trace-openai-chat-provider-retry-override-123")
        .await
        .expect("request candidate trace should read");
    ...
    assert_eq!(
        attempts,
        vec![
            (0, 0, RequestCandidateStatus::Failed, Some(401)),
            (1, 0, RequestCandidateStatus::Failed, Some(401)),
            (1, 1, RequestCandidateStatus::Success, Some(200)),
        ]
    );
