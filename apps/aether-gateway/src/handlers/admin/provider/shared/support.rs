...
const PROVIDER_QUOTA_WINDOW_MAX_DURATION_SECS: u64 = 366 * 24 * 60 * 60;
...
pub(crate) fn normalize_provider_quota_windows(value: Option<&Value>) -> Result<Value, String> {
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        {
            return Err(format!(
                "quota_windows[{index}].duration_secs 必须在 {PROVIDER_QUOTA_WINDOW_MIN_DURATION_SECS} 到 {PROVIDER_QUOTA_WINDOW_MAX_DURATION_SECS} 秒之间"
            ));
        }
