...
增量迁移 `20260908010000_normalize_legacy_policy_nulls.sql` 将这些已知的旧版空值转换成 SQL `NULL`，不修改既有迁移及其校验和。PostgreSQL、MySQL 和 SQLite 的新安装及已有数据库升级均通过各自的迁移机制执行。本分支保留已有的 `20260908000000_add_stats_hourly_model_provider.sql`，因此空值修复使用后一个版本号。
...
共 5 张表、14 个字段，兼容 PostgreSQL 的 `json`、`jsonb` 列，以及 MySQL、SQLite 的 JSON 文本列。迁移可重复执行，且只更新命中旧版空值的字段：
...
- MySQL、SQLite 文本列中的无效 JSON 保持原样，仍由严格读取逻辑拒绝。
