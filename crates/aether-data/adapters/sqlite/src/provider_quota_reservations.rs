...
const PROVIDER_SQL: &str = r#"SELECT p.is_active, CAST(p.billing_type AS TEXT) AS billing_type,
 (p.quota_last_reset_at / 60) * 60 AS epoch, p.quota_expires_at AS expires_at, p.quota_subscription_started_at AS subscription_start,
 CAST(p.quota_reset_day AS BIGINT) AS quota_reset_day, p.quota_cycle_start_at, p.pending_quota_reset_at,
 CAST(p.monthly_quota_usd AS REAL) AS limit_usd, CAST(p.config AS TEXT) AS config
...
const CYCLE_SQL: &str = r#"SELECT CAST(COALESCE(q.monthly_used_usd, 0) + COALESCE((SELECT SUM(d.provider_quota_cost_usd) FROM usage_counter_deltas d
 ...
 ), 0) + COALESCE((SELECT CAST(SUM(r.reserved_cost_units) AS REAL) / 100000000.0 FROM provider_quota_reservations r
 ...
 AND d.request_id = r.candidate_id AND d.quota_accounting_status IN ('ready', 'reconciled'))), 0) AS REAL) AS used_usd
 FROM (SELECT p.id AS provider_id, p.monthly_used_usd, (p.quota_last_reset_at / 60) * 60 AS epoch FROM providers p WHERE p.id = ?) q"#;
const WINDOW_SQL: &str = r#"SELECT w.status, CAST(w.used_usd
 ...
 AND b.bucket_start >= MAX(w.accounted_until, MAX(w.quota_epoch_start, n.clock - w.duration_secs)) AND b.bucket_start < n.clock + 60), 0)
 - COALESCE((SELECT SUM(b.used_usd) FROM provider_quota_usage_buckets b
 WHERE b.provider_id = w.provider_id AND b.quota_epoch_start = w.quota_epoch_start
 AND b.bucket_start >= w.rolling_start AND b.bucket_start < MAX(w.quota_epoch_start, n.clock - w.duration_secs) AND b.bucket_start < w.accounted_until), 0)
 ...
 AND d.provider_dispatch_at_unix_secs >= MAX(w.quota_epoch_start, n.clock - w.duration_secs) AND d.provider_dispatch_at_unix_secs < n.clock + 60), 0) + COALESCE((SELECT CAST(SUM(r.reserved_cost_units) AS REAL) / 100000000.0 FROM provider_quota_reservations r
 WHERE r.provider_id = w.provider_id AND r.quota_epoch_start = w.quota_epoch_start AND r.state IN ('reserved', 'uncertain')
 AND r.dispatch_at >= MAX(w.quota_epoch_start, n.clock - w.duration_secs) AND r.dispatch_at < n.clock + 60
 AND NOT EXISTS (SELECT 1 FROM usage_counter_deltas d WHERE d.kind = 'provider_monthly'
 AND d.request_id = r.candidate_id AND d.quota_accounting_status IN ('ready', 'reconciled'))), 0) AS REAL) AS used_usd
 FROM provider_quota_window_counters w CROSS JOIN (SELECT CAST(? AS BIGINT) AS clock) n
