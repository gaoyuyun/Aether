...
WHERE provider_id = ? AND duration_secs = ? AND quota_epoch_start = ?
  AND status = 'ready'
  AND (accounted_until > ? OR rolling_start > ? OR accounted_until < rolling_start)
"#,
            )
            .bind(now_unix_secs as i64)
            .bind(&provider_id)
      ...
      AND NOT (
        ...
        AND EXISTS (
          ...
          WHERE current_provider.id = usage_counter_deltas.target_id
            AND (current_provider.quota_last_reset_at / 60) * 60
...
INSERT INTO providers (
  ...
  'future-window-provider', 'future window provider', 'custom', 'monthly_quota', 2.5,
  ?, '{"quota_windows":[{"duration_secs":86400,"limit_usd":10}]}', ?, ?
);
INSERT INTO provider_quota_usage_buckets (
  provider_id, quota_epoch_start, bucket_start, used_usd, updated_at
) VALUES ('future-window-provider', ?, ?, 2.5, ?);
"#,
        )
        .bind(epoch as i64)
        .bind(now as i64)
        .bind(now as i64)
        .bind(epoch as i64)
        .bind(clock_minute.saturating_sub(60) as i64)
        .bind(now as i64)
        .execute(&pool)
        .await
        .expect("future window fixtures should seed");

        maintain_provider_quota_windows(&pool, now)
            .await
            .expect("initial window should build");
        let corrupt_time = 3_747_880_460_473_073_640i64;
        sqlx::query(
            "UPDATE provider_quota_window_counters SET rolling_start = ?, accounted_until = ?, status = 'ready' WHERE provider_id = 'future-window-provider'",
        )
        .bind(corrupt_time - 86_400)
        .bind(corrupt_time)
        .execute(&pool)
        .await
        .expect("future counter should corrupt");

        maintain_provider_quota_windows(&pool, now)
            .await
            .expect("future counter should rebuild");
        let repaired: (i64, i64, f64, String) = sqlx::query_as(
            "SELECT rolling_start, accounted_until, used_usd, status FROM provider_quota_window_counters WHERE provider_id = 'future-window-provider' AND duration_secs = 86400",
        )
        .fetch_one(&pool)
        .await
        .expect("repaired counter should load");
        assert_eq!(repaired.0, epoch as i64);
        assert_eq!(repaired.1, clock_minute as i64);
        assert_eq!(repaired.2, 2.5);
        assert_eq!(repaired.3, "ready");
    }
...
INSERT INTO providers (
  ...
  'pending-quota-provider', 'pending quota provider', 'custom', 'monthly_quota',
  10, 0, ?, 1, ?, ?
);
...
INSERT INTO usage_counter_deltas (
  ...
  'pending-quota-delta', 'pending-quota-candidate', 'provider_monthly',
  'pending-quota-provider', 1.25, ?, 'monthly_quota', ?, ?, 1.25, 1,
  'pending', ?
)
"#,
        )
        ...
        .execute(&pool)
        .await
        .expect("pending quota delta should seed");

        let pending_flush = flush(&pool, 100)
            .await
            .expect("pending quota flush should run");
        assert_eq!(pending_flush.rows_claimed, 0);
        let processed_at: Option<i64> = sqlx::query_scalar(
            "SELECT processed_at FROM usage_counter_deltas WHERE id = 'pending-quota-delta'",
        )
        .fetch_one(&pool)
        .await
        .expect("pending quota delta should load");
        assert_eq!(processed_at, None);

        sqlx::query(
            "UPDATE usage_counter_deltas SET quota_accounting_status = 'ready' WHERE id = 'pending-quota-delta'",
        )
        .execute(&pool)
        .await
        .expect("pending quota delta should reconcile");

        let ready_flush = flush(&pool, 100)
            .await
            .expect("ready quota flush should run");
        assert_eq!(ready_flush.rows_claimed, 1);
        assert_eq!(ready_flush.provider_monthly_targets, 1);
        let monthly_used: f64 = sqlx::query_scalar(
            "SELECT monthly_used_usd FROM providers WHERE id = 'pending-quota-provider'",
        )
        .fetch_one(&pool)
        .await
        .expect("monthly usage should load");
        assert_eq!(monthly_used, 1.25);
    }
