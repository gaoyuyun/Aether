...
fn absorb_usage_cost_savings_summary(
    ...
    part: StoredUsageCostSavingsSummary,
) {
    ...
    target.estimated_full_cost_usd += part.estimated_full_cost_usd;
}

fn sqlite_cost_savings_aggregate_table(query: &UsageCostSavingsSummaryQuery) -> &'static str {
    match (
        query.user_id.is_some(),
        query.provider_name.is_some(),
        query.model.is_some(),
    ) {
        (false, false, false) => "stats_daily_cost_savings",
        (false, true, false) => "stats_daily_cost_savings_provider",
        (false, false, true) => "stats_daily_cost_savings_model",
        (false, true, true) => "stats_daily_cost_savings_model_provider",
        (true, false, false) => "stats_user_daily_cost_savings",
        (true, true, false) => "stats_user_daily_cost_savings_provider",
        (true, false, true) => "stats_user_daily_cost_savings_model",
        (true, true, true) => "stats_user_daily_cost_savings_model_provider",
    }
}
...
"#,
    ...
    async fn read_cost_savings_daily_cutoff_unix_secs(
        &self,
        query: &UsageCostSavingsSummaryQuery,
    ) -> Result<Option<u64>, DataLayerError> {
        let mut builder = QueryBuilder::<Sqlite>::new("SELECT MAX(\"date\") FROM ");
        builder.push(sqlite_cost_savings_aggregate_table(query));
        let mut has_where = false;
        push_sqlite_usage_optional_text_filter(
            &mut builder,
            &mut has_where,
        ...
        latest_date
            .map(|value| {
                u64::try_from(value)
                    ...
                    .map_err(|_| {
                        DataLayerError::UnexpectedValue(
                            "stats cost-savings cutoff date must not be negative".to_string(),
                        )
                    })
            })
            .transpose()
...
SELECT
  COALESCE(SUM(cache_read_tokens), 0) AS cache_read_tokens,
  COALESCE(SUM(CAST(cache_read_cost AS REAL)), 0) AS cache_read_cost_usd,
  COALESCE(SUM(CAST(cache_creation_cost AS REAL)), 0) AS cache_creation_cost_usd,
  COALESCE(SUM(CAST(estimated_full_cost AS REAL)), 0) AS estimated_full_cost_usd
FROM
"#,
        );
        builder.push(sqlite_cost_savings_aggregate_table(query));
        let mut has_where = false;
        push_sqlite_usage_where(&mut builder, &mut has_where);
        builder.push("\"date\" >= ").push_bind(to_i64(
            query.created_from_unix_secs,
            "cost savings aggregate start",
        )?);
        push_sqlite_usage_where(&mut builder, &mut has_where);
        builder.push("\"date\" < ").push_bind(to_i64(
            query.created_until_unix_secs,
            "cost savings aggregate end",
        )?);
        push_sqlite_usage_optional_text_filter(
            &mut builder,
            &mut has_where,
    ...
    async fn summarize_usage_cost_savings_raw(
        &self,
        query: &UsageCostSavingsSummaryQuery,
    ) -> Result<StoredUsageCostSavingsSummary, DataLayerError> {
        let mut builder = QueryBuilder::<Sqlite>::new(format!(
...
AND LOWER(TRIM(COALESCE(provider_name, ''))) NOT IN ('unknown', 'pending')",
    ...
    async fn summarize_usage_cost_savings(
        ...
        let Some(cutoff) = self.read_cost_savings_daily_cutoff_unix_secs(query).await? else {
            return self.summarize_usage_cost_savings_raw(query).await;
        };
        let aggregate_start = query.created_from_unix_secs.saturating_add(86_399) / 86_400 * 86_400;
        let aggregate_end = query.created_until_unix_secs.min(cutoff) / 86_400 * 86_400;
        if aggregate_start >= aggregate_end {
        ...
        if query.created_from_unix_secs < aggregate_start {
            absorb_usage_cost_savings_summary(
                &mut summary,
                self.summarize_usage_cost_savings_raw(&UsageCostSavingsSummaryQuery {
                    created_from_unix_secs: query.created_from_unix_secs,
                    created_until_unix_secs: aggregate_start,
                    ..query.clone()
                })
