...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Note: the pre-fix run (raw merge) may report more rows than the fast path
    // because the pre-fix daily-aggregate merge double-counted boundary-day
    // usage (whole UTC days mislabeled into single local days). The fast path's
    // row-for-row numeric equality with raw aggregation is asserted by the
    // repository test suite
    // (`sqlite_dashboard_daily_breakdown_hourly_fast_path_matches_raw_across_timezones`).
    let equal = !fast_rows.is_empty();
    ...
    println!(
        "{:<44} {:>12} {:>10}",
        "fast path returned rows", "-", if equal { "PASS" } else { "FAIL" }
    );
    println!(
        "{:<44} {:>12.1} {:>10}",
