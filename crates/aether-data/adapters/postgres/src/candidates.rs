...
WHERE delta.id = $1 AND delta.kind = 'provider_monthly'
  AND delta.processed_at IS NULL
  AND delta.quota_accounting_status IN ('pending', 'failed')
  AND usage_record.request_id = $2
  AND (
    routing.candidate_id = $3
    OR (routing.candidate_id IS NULL AND usage_record.provider_id = $4 AND $5)
  )
