...
use crate::{
    ...
    PostgresTransaction, PostgresTransactionRunner,
};
use aether_data_contracts::repository::usage::{
    ...
    ProviderApiKeyUsageDelta, ProviderApiKeyWindowUsageRequest, ProviderQuotaWindowUsageRequest,
...
SELECT
  ...
  CAST(delta.provider_billing_type_at_usage AS TEXT) AS provider_billing_type_at_usage,
  delta.quota_epoch_start_at_usage,
  delta.provider_dispatch_at_unix_secs,
  delta.provider_quota_cost_usd,
  delta.quota_delta_sequence,
  delta.quota_accounting_status,
  CAST(EXTRACT(EPOCH FROM delta.created_at) AS BIGINT) AS created_at_unix_secs
    ...
    AND NOT (
      ...
      AND EXISTS (
        ...
        WHERE current_provider.id = usage_counter_deltas.target_id
          AND CAST(
            FLOOR(EXTRACT(EPOCH FROM current_provider.quota_last_reset_at) / 60) * 60
            AS BIGINT
...
"#,
    ...
    /// Hourly model × provider rows regrouped into tz-local days.
    ///
    /// A local day under a fixed offset maps to a contiguous UTC hour range,
    /// so `stats_hourly_model_provider` rows regroup exactly into local days
    /// regardless of tz (30/45-minute offsets included).
    async fn list_dashboard_daily_breakdown_from_hourly_model_provider(
        &self,
        query: &UsageDashboardDailyBreakdownQuery,
        aggregate_start: DateTime<Utc>,
...
SELECT
  ...
  SUM(input_tokens + output_tokens + cache_creation_tokens + cache_read_tokens)::BIGINT
    AS total_tokens,
  SUM(total_cost)::DOUBLE PRECISION AS total_cost_usd,
  SUM(response_time_sum_ms)::DOUBLE PRECISION AS response_time_sum_ms,
  SUM(response_time_samples)::BIGINT AS response_time_samples
FROM stats_hourly_model_provider
WHERE hour_utc >= "#,
        );
        builder.push_bind(aggregate_start);
        builder.push(" AND hour_utc < ");
        builder.push_bind(aggregate_end);
        builder.push(
            r#"
...
"#,
    ...
    pub async fn list_dashboard_daily_breakdown(
        ...
        if query.tz_offset_minutes != 0 {
            // Local days regroup exactly from complete UTC hours, so serve them
            // from stats_hourly_model_provider and only scan raw usage for the
            // local days the uncovered tail touches. Fall back to raw when
            // hourly coverage is missing (fresh deployments before backfill).
            if let Ok(Some(cutoff_utc)) = self.read_stats_hourly_cutoff().await {
                ...
                if split.aggregate.is_some() {
                    ...
                    // days from there so no day is served from both sources.
                    let hourly_until = dashboard_utc_midnight(cutoff_utc + tz_minutes) - tz_minutes;
                    let hourly_until = hourly_until.min(local_end);
                    if local_start < hourly_until {
                        let mut items = self
                            .list_dashboard_daily_breakdown_from_hourly_model_provider(
                                query,
                                local_start,
                                hourly_until,
                            )
                        ...
                        // Raw rebuilds every local day the hourly segment could
                        // not cover: the not-yet-aggregated tail from the cutoff.
                        if hourly_until < local_end {
                            for item in self
                                .list_dashboard_daily_breakdown_raw(
                                    &UsageDashboardDailyBreakdownQuery {
                                        ...
                                        created_until_unix_secs: dashboard_utc_to_unix_secs(
                                            local_end,
                                        ),
                                ...
                                .await?
                            {
                                if !covered_dates.contains(&item.date) {
                                    items.push(item);
                                }
                            }
                        }
        ...
        self.tx_runner
            .run_read_write(|tx| {
                Box::pin(async move {
                    ...
                    for row in &rows {
                        if row.kind == USAGE_COUNTER_KIND_PROVIDER_MONTHLY {
...
ON CONFLICT (provider_id, quota_epoch_start) DO UPDATE SET
  applied_delta_sequence = GREATEST(
    provider_quota_applied_watermarks.applied_delta_sequence,
    EXCLUDED.applied_delta_sequence
  ),
  updated_at = NOW()
"#,
                        )
    ...
    pub async fn maintain_provider_quota_windows(
        &self,
        now_unix_secs: u64,
            ...
            .run_read_write(|tx| {
                Box::pin(async move {
                    ...
                    let providers = sqlx::query(
                        r#"
SELECT id,
       CAST(EXTRACT(EPOCH FROM quota_last_reset_at) AS BIGINT) AS quota_epoch_start,
       config
FROM providers
...
"#,
                    ...
                    for provider in providers {
                        let provider_id: String = provider.try_get("id").map_postgres_err()?;
                        ...
                        if !process_provider_quota_backfill_postgres(
                            tx,
                            &provider_id,
                            epoch,
                            clock_minute,
...
SET status = 'rebuilding', rebuild_error = NULL, updated_at = NOW()
WHERE provider_id = $1
  AND status = 'failed'
  ...
  AND NOT EXISTS (
    ...
    WHERE kind = 'provider_monthly'
      AND target_id = $1
      AND quota_epoch_start_at_usage = $2
      AND quota_accounting_status IN ('pending', 'failed')
  )
"#,
                        )
                        ...
                        let existing = sqlx::query_scalar::<_, i64>(
                            "SELECT duration_secs FROM provider_quota_window_counters WHERE provider_id = $1 FOR UPDATE",
                        )
                        .bind(&provider_id)
                        .fetch_all(&mut **tx)
                        .await
                        .map_postgres_err()?;
                        for duration in existing {
                            if !windows.iter().any(|window| {
                                window.duration_secs == duration.max(0) as u64
                            }) {
                                sqlx::query("DELETE FROM provider_quota_window_counters WHERE provider_id = $1 AND duration_secs = $2")
                                    .bind(&provider_id)
                                    .bind(duration)
                                    .execute(&mut **tx)
                                    .await
                        ...
                        for window in windows {
                            maintain_postgres_quota_window_in_tx(
                                tx,
                                &provider_id,
                                epoch,
...
SELECT COALESCE(SUM(COALESCE(
         NULLIF(settlement.settlement_snapshot ->> 'provider_cost_usd', '')::DOUBLE PRECISION,
...
LEFT JOIN usage_settlement_snapshots AS settlement
  ON settlement.request_id = usage_record.request_id
WHERE usage_record.provider_id = $1
  AND usage_record.created_at >= TO_TIMESTAMP($2::double precision)
"#,
        )
        .bind(provider_id)
        .bind(since_unix_secs as f64)
    ...
    async fn read_provider_quota_window_usage(
        ...
        let mut builder = QueryBuilder::<Postgres>::new(
            "SELECT provider_id, duration_secs, CAST(EXTRACT(EPOCH FROM quota_epoch_start) AS BIGINT) AS quota_epoch_start_unix_secs, CAST(EXTRACT(EPOCH FROM rolling_start) AS BIGINT) AS rolling_start_unix_secs, CAST(EXTRACT(EPOCH FROM accounted_until) AS BIGINT) AS accounted_until_unix_secs, used_usd, status, rebuild_error FROM provider_quota_window_counters WHERE provider_id IN (",
        );
        {
            let mut separated = builder.separated(", ");
        ...
        for row in rows {
            let usage = StoredProviderQuotaWindowUsage {
                ...
                duration_secs: row
                    .try_get::<i64, _>("duration_secs")
                    .map_postgres_err()?
                    .max(0) as u64,
                quota_epoch_start_unix_secs: row
                    .try_get::<i64, _>("quota_epoch_start_unix_secs")
                    .map_postgres_err()?
                    .max(0) as u64,
                rolling_start_unix_secs: row
                    .try_get::<i64, _>("rolling_start_unix_secs")
                    .map_postgres_err()?
                    .max(0) as u64,
                accounted_until_unix_secs: row
                    .try_get::<i64, _>("accounted_until_unix_secs")
                    .map_postgres_err()?
                    .max(0) as u64,
                used_usd: row.try_get("used_usd").map_postgres_err()?,
                status: row.try_get("status").map_postgres_err()?,
                rebuild_error: row.try_get("rebuild_error").map_postgres_err()?,
            };
...
impl UsageWriteRepository for SqlxUsageReadRepository {
    ...
    async fn maintain_provider_quota_windows(
        ...
        Self::maintain_provider_quota_windows(self, now_unix_secs).await
    }
...
async fn process_provider_quota_backfill_postgres(
...
WHERE provider_id = $1 AND quota_epoch_start = $2 AND task_kind = 'historical_backfill'
LIMIT 1 FOR UPDATE
"#,
    )
    .bind(provider_id)
    ...
    let status: String = task.try_get("status").map_postgres_err()?;
    if status == "complete" {
        return Ok(true);
    }
    if status == "failed" {
    ...
    let cursor_dispatch_at: i64 = task.try_get("cursor_dispatch_at").map_postgres_err()?;
    let cursor_request_id: String = task.try_get("cursor_request_id").map_postgres_err()?;
    let cutover_sequence = task
        .try_get::<Option<i64>, _>("cutover_delta_sequence")
        .map_postgres_err()?;
    let lock_owner = Uuid::new_v4().to_string();
    sqlx::query(
        "UPDATE provider_quota_maintenance_state SET status = 'running', lock_owner = $1, lock_expires_at = $2, updated_at = NOW() WHERE provider_id = $3 AND quota_epoch_start = $4 AND task_kind = 'historical_backfill'",
    )
    .bind(&lock_owner)
    .bind(chrono::Utc::now().timestamp().saturating_add(30))
    .bind(provider_id)
    .bind(epoch as i64)
    .execute(&mut **tx)
    .await
    ...
    if status == "pending" {
        sqlx::query(
            "DELETE FROM provider_quota_usage_buckets WHERE provider_id = $1 AND quota_epoch_start = TO_TIMESTAMP($2::double precision)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
        .execute(&mut **tx)
        ...
        .map_postgres_err()?;
        sqlx::query(
            "UPDATE providers SET monthly_used_usd = 0, updated_at = NOW() WHERE id = $1 AND quota_last_reset_at = TO_TIMESTAMP($2::double precision)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
        .execute(&mut **tx)
...
WITH candidate_rows AS (
  SELECT
    ...
    FLOOR(EXTRACT(EPOCH FROM usage_record.created_at))::bigint AS created_at_unix_secs,
    COALESCE(
      NULLIF(candidate.extra_data #>> '{provider_quota_dispatch_snapshot,provider_dispatch_at_unix_secs}', '')::bigint,
      FLOOR(EXTRACT(EPOCH FROM candidate.started_at))::bigint
    ) AS dispatch_at,
    COALESCE(
      candidate.extra_data #>> '{provider_quota_dispatch_snapshot,provider_billing_type_at_usage}',
      snapshot.settlement_snapshot #>> '{pricing_snapshot,provider_billing_type}'
  ...
  SELECT
    request_id,
    COALESCE(dispatch_at, created_at_unix_secs) AS scan_at,
    dispatch_at,
...
"#,
    ...
    if rows.is_empty() {
        let pending_count = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM usage_counter_deltas WHERE kind = 'provider_monthly' AND target_id = $1 AND quota_epoch_start_at_usage = $2 AND quota_accounting_status IN ('pending', 'failed') AND ($3::bigint IS NULL OR quota_delta_sequence <= $3)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
        ...
        .map_postgres_err()?;
        if pending_count > 0 {
            sqlx::query("UPDATE provider_quota_maintenance_state SET lock_owner = NULL, lock_expires_at = NULL, updated_at = NOW() WHERE provider_id = $1 AND quota_epoch_start = $2 AND task_kind = 'historical_backfill'")
                .bind(provider_id)
                .bind(epoch as i64)
                .execute(&mut **tx)
                .await
        ...
        if let Some(cutover_sequence) = cutover_sequence {
            sqlx::query("UPDATE usage_counter_deltas SET processed_at = COALESCE(processed_at, NOW()), quota_accounting_status = 'backfill_absorbed' WHERE kind = 'provider_monthly' AND target_id = $1 AND quota_epoch_start_at_usage = $2 AND quota_delta_sequence <= $3")
                .bind(provider_id)
                .bind(epoch as i64)
                .bind(cutover_sequence)
                .execute(&mut **tx)
                .await
                .map_postgres_err()?;
            sqlx::query("INSERT INTO provider_quota_applied_watermarks (provider_id, quota_epoch_start, applied_delta_sequence, updated_at) VALUES ($1, $2, $3, NOW()) ON CONFLICT (provider_id, quota_epoch_start) DO UPDATE SET applied_delta_sequence = GREATEST(provider_quota_applied_watermarks.applied_delta_sequence, EXCLUDED.applied_delta_sequence), updated_at = NOW()")
                .bind(provider_id)
                .bind(epoch as i64)
                .bind(cutover_sequence)
                .execute(&mut **tx)
        ...
        let total_used = sqlx::query_scalar::<_, f64>(
            "SELECT COALESCE(SUM(used_usd), 0)::double precision FROM provider_quota_usage_buckets WHERE provider_id = $1 AND quota_epoch_start = TO_TIMESTAMP($2::double precision)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
        .fetch_one(&mut **tx)
        .await
        .map_postgres_err()?;
        sqlx::query("UPDATE providers SET monthly_used_usd = $1, updated_at = NOW() WHERE id = $2 AND quota_last_reset_at = TO_TIMESTAMP($3::double precision)")
            .bind(total_used)
            .bind(provider_id)
            .bind(epoch as i64)
            .execute(&mut **tx)
            .await
            .map_postgres_err()?;
        let unknown_rows: i64 = task.try_get("unknown_rows").map_postgres_err()?;
        ...
        sqlx::query("UPDATE provider_quota_maintenance_state SET status = $1, absorbed_delta_sequence = COALESCE(cutover_delta_sequence, 0), lock_owner = NULL, lock_expires_at = NULL, last_error = $2, updated_at = NOW() WHERE provider_id = $3 AND quota_epoch_start = $4 AND task_kind = 'historical_backfill'")
            .bind(final_status)
            .bind(last_error)
            .bind(provider_id)
            .bind(epoch as i64)
        ...
        if final_status == "failed" {
            sqlx::query("UPDATE provider_quota_window_counters SET status = 'failed', rebuild_error = $1, updated_at = NOW() WHERE provider_id = $2 AND quota_epoch_start = TO_TIMESTAMP($3::double precision)")
                .bind(last_error)
                .bind(provider_id)
                .bind(epoch as i64)
                .execute(&mut **tx)
    ...
    for row in rows {
        let request_id: String = row.try_get("request_id").map_postgres_err()?;
        let scan_at: i64 = row.try_get("scan_at").map_postgres_err()?;
        ...
        match billing_type.as_deref() {
            ...
            Some("monthly_quota") => {
                let dispatch_at = row
                    .try_get::<Option<i64>, _>("dispatch_at")
                    .map_postgres_err()?;
                let cost = row
    ...
    for (bucket_start, used_usd) in buckets {
        sqlx::query("INSERT INTO provider_quota_usage_buckets (provider_id, quota_epoch_start, bucket_start, used_usd, updated_at) VALUES ($1, TO_TIMESTAMP($2::double precision), TO_TIMESTAMP($3::double precision), $4, NOW()) ON CONFLICT (provider_id, quota_epoch_start, bucket_start) DO UPDATE SET used_usd = provider_quota_usage_buckets.used_usd + EXCLUDED.used_usd, updated_at = NOW()")
            .bind(provider_id)
            .bind(epoch as i64)
    ...
    sqlx::query("UPDATE provider_quota_maintenance_state SET cursor_dispatch_at = $1, cursor_request_id = $2, included_rows = included_rows + $3, excluded_payg_rows = excluded_payg_rows + $4, excluded_free_tier_rows = excluded_free_tier_rows + $5, unknown_rows = unknown_rows + $6, lock_owner = NULL, lock_expires_at = NULL, updated_at = NOW() WHERE provider_id = $7 AND quota_epoch_start = $8 AND task_kind = 'historical_backfill'")
        .bind(last_scan_at)
...
fn map_usage_counter_delta_row(row: &PgRow) -> Result<UsageCounterDeltaRow, DataLayerError> {
    Ok(UsageCounterDeltaRow {
        ...
        provider_billing_type_at_usage: row
            ...
            .map_postgres_err()?,
        quota_epoch_start_at_usage: optional_i64_to_unix_secs(
            "usage_counter_deltas.quota_epoch_start_at_usage",
            row.try_get("quota_epoch_start_at_usage")
                .map_postgres_err()?,
        )?,
        provider_dispatch_at_unix_secs: optional_i64_to_unix_secs(
            "usage_counter_deltas.provider_dispatch_at_unix_secs",
            row.try_get("provider_dispatch_at_unix_secs")
                .map_postgres_err()?,
        )?,
        provider_quota_cost_usd: row.try_get("provider_quota_cost_usd").map_postgres_err()?,
        quota_delta_sequence: row.try_get("quota_delta_sequence").map_postgres_err()?,
        quota_accounting_status: row.try_get("quota_accounting_status").map_postgres_err()?,
        created_at_unix_secs: row
            .try_get::<i64, _>("created_at_unix_secs")
            .map_postgres_err()?
            .max(0) as u64,
...
async fn apply_provider_monthly_usage_delta_in_tx(
    ...
    if provider_id.trim().is_empty() || deltas.is_empty() {
        return Ok(());
    }
...
"#,
    ...
    let current_epoch = optional_i64_to_unix_secs(
        "providers.quota_last_reset_at",
        provider
            .try_get("quota_last_reset_at_unix_secs")
            .map_postgres_err()?,
    )?
    ...
    if current_epoch_delta != 0.0 {
        sqlx::query(
            r#"
UPDATE providers
SET monthly_used_usd = COALESCE(monthly_used_usd, 0) + $2,
    updated_at = NOW()
WHERE id = $1
  AND quota_last_reset_at = TO_TIMESTAMP($3::double precision)
"#,
        )
        .bind(provider_id)
        .bind(current_epoch_delta)
        .bind(current_epoch.map(|value| value as f64))
        .execute(&mut **tx)
        .await
        .map_postgres_err()?;
    }
...
) VALUES ($1, TO_TIMESTAMP($2::double precision), TO_TIMESTAMP($3::double precision), $4, NOW())
...
"#,
            ...
            .bind(provider_id)
            .bind(delta.quota_epoch_start_unix_secs as f64)
...
INSERT INTO provider_quota_window_counters (
  ...
  $1, $2, TO_TIMESTAMP($3::double precision), TO_TIMESTAMP($4::double precision),
  TO_TIMESTAMP($5::double precision), 0, 'rebuilding', NULL, NOW()
)
...
"#,
            ...
            .bind(delta.quota_epoch_start_unix_secs as f64)
            .bind(rolling_start as f64)
            .bind(quota_clock_minute(now) as f64)
            .execute(&mut **tx)
            .await
            .map_postgres_err()?;
            if delta.accounting_ready {
...
WHERE provider_id = $2 AND duration_secs = $3
  AND quota_epoch_start = TO_TIMESTAMP($4::double precision)
  AND status = 'ready'
  AND TO_TIMESTAMP($5::double precision) >= rolling_start
  AND TO_TIMESTAMP($5::double precision) < accounted_until
"#,
                )
                .bind(delta.provider_quota_cost_usd)
                .bind(provider_id)
...
SET status = 'failed',
    ...
    updated_at = NOW()
WHERE provider_id = $1 AND duration_secs = $2
  AND quota_epoch_start = TO_TIMESTAMP($3::double precision)
"#,
                )
                .bind(provider_id)
                .bind(window.duration_secs as i64)
...
async fn maintain_postgres_quota_window_in_tx(
    tx: &mut sqlx::Transaction<'_, Postgres>,
    provider_id: &str,
    epoch: u64,
    duration_secs: u64,
...
"#,
    ...
    .bind(epoch as f64)
    .bind(desired_start as f64)
    .bind(clock_minute as f64)
    .execute(&mut **tx)
    .await
    .map_postgres_err()?;
...
WHERE provider_id = $1 AND duration_secs = $2
  ...
  AND status = 'ready'
  AND (
    accounted_until > TO_TIMESTAMP($4::double precision)
    OR rolling_start > TO_TIMESTAMP($5::double precision)
    OR accounted_until < rolling_start
  )
"#,
    )
    .bind(provider_id)
...
SET rolling_start = TO_TIMESTAMP($4::double precision),
    accounted_until = TO_TIMESTAMP($5::double precision),
    used_usd = COALESCE((
      SELECT SUM(bucket.used_usd)
      FROM provider_quota_usage_buckets AS bucket
      WHERE bucket.provider_id = $1
        AND bucket.quota_epoch_start = TO_TIMESTAMP($3::double precision)
        AND bucket.bucket_start >= TO_TIMESTAMP($4::double precision)
        AND bucket.bucket_start < TO_TIMESTAMP($5::double precision)
    ), 0),
    status = 'ready', rebuild_error = NULL, updated_at = NOW()
WHERE counter.provider_id = $1 AND counter.duration_secs = $2
  AND counter.quota_epoch_start = TO_TIMESTAMP($3::double precision)
  AND counter.status = 'rebuilding'
"#,
    )
    .bind(provider_id)
    .bind(duration_secs as i64)
    ...
    let row = sqlx::query(
        r#"
SELECT CAST(EXTRACT(EPOCH FROM rolling_start) AS BIGINT) AS rolling_start,
       CAST(EXTRACT(EPOCH FROM accounted_until) AS BIGINT) AS accounted_until,
       used_usd
...
"#,
    ...
    let accounted_until = row
        ...
        .map_postgres_err()?
        .max(0) as u64;
    let mut used_usd: f64 = row.try_get("used_usd").map_postgres_err()?;
    if accounted_until < clock_minute {
        let added: f64 = sqlx::query_scalar(
            r#"
SELECT COALESCE(SUM(used_usd), 0)::DOUBLE PRECISION
FROM provider_quota_usage_buckets
WHERE provider_id = $1 AND quota_epoch_start = TO_TIMESTAMP($2::double precision)
  AND bucket_start >= TO_TIMESTAMP($3::double precision)
  AND bucket_start < TO_TIMESTAMP($4::double precision)
"#,
        )
        .bind(provider_id)
        .bind(epoch as f64)
        .bind(accounted_until as f64)
        .bind(clock_minute as f64)
        .fetch_one(&mut **tx)
        .await
        .map_postgres_err()?;
        used_usd += added;
        sqlx::query(
            "UPDATE provider_quota_window_counters SET used_usd = $1, accounted_until = TO_TIMESTAMP($2::double precision), updated_at = NOW() WHERE provider_id = $3 AND duration_secs = $4 AND quota_epoch_start = TO_TIMESTAMP($5::double precision) AND status = 'ready' AND accounted_until = TO_TIMESTAMP($6::double precision)",
        )
        .bind(used_usd)
        .bind(clock_minute as f64)
        .bind(provider_id)
...
"#,
        ...
        .map_postgres_err()?;
        sqlx::query(
            "UPDATE provider_quota_window_counters SET used_usd = GREATEST($1 - $2, 0), rolling_start = TO_TIMESTAMP($3::double precision), updated_at = NOW() WHERE provider_id = $4 AND duration_secs = $5 AND quota_epoch_start = TO_TIMESTAMP($6::double precision) AND status = 'ready' AND rolling_start = TO_TIMESTAMP($7::double precision)",
        )
        .bind(used_usd)
        .bind(removed)
        .bind(desired_start as f64)
...
fn shallow_usage_body_projection_sql(sql: &str) -> String {
    let replacements = [
        ...
        (
            ...
            "CASE WHEN \"usage\".request_body IS NULL AND \"usage\".request_body_compressed IS NULL THEN NULL ELSE ''::bytea END AS request_body_compressed,",
        ),
        (
        ...
        (
            ...
            "CASE WHEN \"usage\".provider_request_body IS NULL AND \"usage\".provider_request_body_compressed IS NULL THEN NULL ELSE ''::bytea END AS provider_request_body_compressed,",
        ),
        ("\"usage\".response_body,", "NULL::json AS response_body,"),
        (
            "\"usage\".response_body_compressed,",
            "CASE WHEN \"usage\".response_body IS NULL AND \"usage\".response_body_compressed IS NULL THEN NULL ELSE ''::bytea END AS response_body_compressed,",
        ),
        (
        ...
        (
            ...
            "CASE WHEN \"usage\".client_response_body IS NULL AND \"usage\".client_response_body_compressed IS NULL THEN NULL ELSE ''::bytea END AS client_response_body_compressed,",
        ),
    ];
