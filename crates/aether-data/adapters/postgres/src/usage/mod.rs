...
"#,
    ...
    if rows.is_empty() {
        let pending_count = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM usage_counter_deltas WHERE kind = 'provider_monthly' AND target_id = $1 AND quota_epoch_start_at_usage = $2 AND quota_accounting_status = 'pending' AND ($3::bigint IS NULL OR quota_delta_sequence <= $3)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
