...
ON DUPLICATE KEY UPDATE
  used_usd = IF(window_start = VALUES(window_start), used_usd + VALUES(used_usd), VALUES(used_usd)),
...
"#,
        ...
        .bind(to_i64(
            ...
            "provider quota window duration",
        )?)
        .bind(to_i64(window_start, "provider quota window start")?)
        .bind(window_delta)
        .bind(now_db)
        .execute(&mut **tx)
        .await
