...
"#,
    ...
    if rows.is_empty() {
        let pending_count = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM usage_counter_deltas WHERE kind = 'provider_monthly' AND target_id = ? AND quota_epoch_start_at_usage = ? AND quota_accounting_status = 'pending' AND (? IS NULL OR quota_delta_sequence <= ?)",
        )
        .bind(provider_id)
        .bind(epoch as i64)
