    ...
    loop {
        ...
        for row in rows {
            let metadata: Option<String> = sqlx::query_scalar(
                "SELECT request_metadata FROM `usage` WHERE id = ? LIMIT 1 FOR UPDATE",
            )
            .bind(&row.id)
...
async fn externalize_detail_bodies(
    ...
    loop {
        let rows = fetch_cleanup_rows(
            pool,
            INLINE_OR_COMPRESSED_BODY_PREDICATE,
            cutoff,
            newer_than,
            batch_size,
        )
        ...
        for candidate in rows {
            let mut tx = pool.begin().await.map_sql_err()?;
            // Read the source after acquiring the write lock so a concurrent
            // usage update cannot be replaced by an older body snapshot.
            let Some(row) = fetch_body_row(&mut tx, &candidate.id).await? else {
                tx.commit().await.map_sql_err()?;
                continue;
            };
...
async fn fetch_body_row(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    id: &str,
) -> Result<Option<BodyRow>, DataLayerError> {
    let sql = format!(
        r#"
SELECT id,
       request_id,
...
FROM `usage`
WHERE id = ? AND ({INLINE_OR_COMPRESSED_BODY_PREDICATE}) FOR UPDATE
"#
    );
