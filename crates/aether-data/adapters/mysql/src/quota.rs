...
fn quota_snapshot_select() -> SelectQuery<'static> {
    SelectQuery::new("providers").select_columns([
        ...
        SelectColumn::expr(
            ...
            .with_mysql(
                "CASE WHEN is_active = 1 AND NOT EXISTS (SELECT 1 FROM provider_quota_maintenance_state AS task WHERE task.provider_id = providers.id AND task.quota_epoch_start = (providers.quota_last_reset_at DIV 60) * 60 AND task.status IN ('pending', 'running', 'failed')) AND NOT EXISTS (SELECT 1 FROM usage_counter_deltas AS delta WHERE delta.kind = 'provider_monthly' AND delta.target_id = providers.id AND delta.quota_epoch_start_at_usage = (providers.quota_last_reset_at DIV 60) * 60 AND delta.quota_accounting_status IN ('pending', 'failed') AND NOT EXISTS (SELECT 1 FROM provider_quota_reservations AS reservation WHERE reservation.candidate_id = delta.request_id AND reservation.provider_id = delta.target_id AND reservation.quota_epoch_start = delta.quota_epoch_start_at_usage AND reservation.state IN ('reserved', 'uncertain'))) THEN 1 ELSE 0 END",
            ),
        )
        .alias("is_active"),
    ])
...
impl ProviderQuotaWriteRepository for MysqlProviderQuotaRepository {
  ...
  (SELECT CAST(JSON_UNQUOTE(JSON_EXTRACT(ss.settlement_snapshot, '$.provider_quota_cost_usd')) AS DOUBLE) FROM usage_settlement_snapshots ss
    ...
    WHERE routing.candidate_id = c.id AND ss.finalized_at IS NOT NULL AND JSON_UNQUOTE(JSON_EXTRACT(ss.settlement_snapshot, '$.status')) = 'complete' LIMIT 1) AS settled_cost
