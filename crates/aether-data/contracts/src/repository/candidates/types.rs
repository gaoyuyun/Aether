...
pub fn sanitize_request_candidate_extra_data_for_persistence(
    ...
    let mut sanitized = sanitize_request_candidate_extra_data(extra_data.clone())
        ...
        .unwrap_or_default();
    for (key, fields) in [
        // Server-generated quota accounting must survive candidate persistence and reads.
        (
            "provider_quota_attempt_accounting",
            &["status", "policy", "cost_usd", "settlement"][..],
        ),
        (
            "provider_quota_recovery",
            &["delta_id", "reason", "cost_usd", "recovered_at_unix_secs"][..],
        ),
...
pub fn sanitize_request_candidate_extra_data(
    ...
    // This server-generated snapshot is used by quota reconciliation and must
    // survive both persistence and subsequent candidate reads.
    if let Some(snapshot) = object
        .get(PROVIDER_QUOTA_DISPATCH_SNAPSHOT_KEY)
        .and_then(|value| {
            serde_json::from_value::<ProviderQuotaDispatchSnapshot>(value.clone()).ok()
        })
        .filter(|snapshot| snapshot.validate().is_ok())
        .and_then(|snapshot| serde_json::to_value(snapshot).ok())
    {
        sanitized.insert(PROVIDER_QUOTA_DISPATCH_SNAPSHOT_KEY.to_string(), snapshot);
    }
    if object
        ...
        == Some("request_terminal")
    {
        sanitized.insert(
            ...
            serde_json::json!("request_terminal"),
        );
    }
