...
fn empty_database_snapshot_sql_includes_usage_body_blobs_and_audit_admin_role() {
    ...
    assert!(EMPTY_DATABASE_SNAPSHOT_SQL.contains("provider_quota_window_counters"));
...
fn pending_migrations_from_applied_keeps_post_snapshot_incrementals_pending() {
    ...
    let expected = all_up_migrations()
        .into_iter()
        .filter(|migration| {
            migration.version > EMPTY_DATABASE_SNAPSHOT_CUTOFF_VERSION
                || super::super::bootstrap::postgres::FORK_POST_SNAPSHOT_MIGRATIONS
                    .contains(&migration.version)
        })
        .collect::<Vec<_>>();

    assert_eq!(pending, expected);
    assert!(pending.iter().all(|migration| migration.version
        > EMPTY_DATABASE_SNAPSHOT_CUTOFF_VERSION
        || super::super::bootstrap::postgres::FORK_POST_SNAPSHOT_MIGRATIONS
            .contains(&migration.version)));
}
