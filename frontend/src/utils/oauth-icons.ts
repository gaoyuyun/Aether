...
export const OAUTH_ICONS: Record<string, string> = {
  ...
  grok_build: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#111827"/><path d="M7 7L17 17M17 7L7 17" stroke="#FFFFFF" stroke-width="2" stroke-linecap="round"/><path d="M12 15.5v3" stroke="#60A5FA" stroke-width="2" stroke-linecap="round"/></svg>`,
}
...
export function getOAuthIconUrl(providerType: string, iconUrl?: string | null): string | null {
  if (OAUTH_ICONS[providerType.toLowerCase()] || !iconUrl?.trim()) return null
  try {
    const parsed = new URL(iconUrl.trim())
