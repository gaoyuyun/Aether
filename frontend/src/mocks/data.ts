...
export const MOCK_SYSTEM_CONFIGS: Array<{ key: string; value: unknown; description?: string }> = [
  ...
  { key: 'module.announcements.enabled', value: false, description: '公告管理模块开关' },
...
const MOCK_MODULE_DEFINITIONS: Array<Omit<ModuleStatus, 'active' | 'health'> & { health?: ModuleStatus['health'] }> = [
  ...
  },
  {
    ...
    config_validated: true,
    config_error: null,
    admin_route: '/admin/announcements',
  ...
  },
  {
    ...
    display_name: '独立密钥',
    description: '管理不绑定用户账号的独立 API 密钥；关闭后全部停止鉴权，但保留密钥、钱包与策略数据',
    category: 'security',
    available: true,
    enabled: false,
    config_validated: true,
