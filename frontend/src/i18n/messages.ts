...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '最大重试次数': 'Max retries',
  '默认 2': 'Default 2',
  '同 Key 重试次数': 'Same-key retries',
  '留空则沿用调度策略': 'Leave empty to follow the routing policy',
  '0 表示失败后直接转移，1 表示再试一次（共两次）；设置后对本供应商的每个 Key 生效': '0 fails over immediately, 1 retries once (two requests in total); applies to every key of this provider',
  ...
  '窗口时长 (小时)': 'Window duration (hours)',
  '周期开始时间（精确到分）': 'Cycle start time (minute precision)',
  ...
  '上游给出重试提示时按提示冷却；没有提示的 429 按 30 秒起步的指数退避，最长 30 分钟。': 'Cool down according to upstream retry hints when present; 429s without a hint use exponential backoff starting at 30 seconds, capped at 30 minutes.',
  '禁用冷却': 'Disable cooldown',
  '关闭后上游失败不再让 Key 进入冷却，仅用于回滚排障。': 'When disabled, upstream failures no longer put keys into cooldown. Only for rollback and troubleshooting.',
  '瞬时错误冷却秒数': 'Transient error cooldown (seconds)',
  '5xx、408、520–526 等瞬时错误后的冷却时长；0 表示这类错误不冷却。': 'Cooldown after transient errors such as 5xx, 408 and 520–526; 0 disables cooldown for them.',
  '模型级冷却': 'Model-level cooldown',
  '开启后只冷却出错的模型，同一把 Key 上的其他模型继续可调度。': 'When enabled, only the failing model is cooled down; other models on the same key stay schedulable.',
  '客户端伪装模式': 'Client cloak mode',
  '原生 Claude Code 请求始终逐字节透传；第三方客户端的请求按所选模式补齐身份、beta、cache_control 与 CCH 签名。': 'Native Claude Code requests are always passed through byte-for-byte; third-party client requests get identity, beta, cache_control and CCH signature applied according to the selected mode.',
  '自动（仅第三方）': 'Auto (third-party only)',
  '总是改写': 'Always rewrite',
  '自动：识别为第三方才改写；总是：所有客户端都改写；关闭：只同步计费头版本。识别结果会写入请求详情的 claude_code_cloak 字段。': 'Auto: rewrite only when the client is detected as third-party; Always: rewrite every client; Off: only sync the billing header version. The detection result is written to the claude_code_cloak field in request details.',
  '传输指纹 profile': 'Transport fingerprint profile',
  '系统默认走 rustls；选择仿真 profile 后出站 TLS 改用 BoringSSL 复刻原生客户端的 ClientHello 与头顺序。Key 级设置优先。': 'The system default uses rustls; selecting an emulation profile switches outbound TLS to BoringSSL, replicating the native client ClientHello and header order. Key-level settings take precedence.',
  '仿真参数以 CLIProxyAPI 复刻的 Claude Code 2.1.220 抓包为起点，尚未用真实抓包核对；可在 Key 编辑里「探测 TLS 指纹」查看实际 JA3/JA4。': 'Emulation parameters start from the Claude Code 2.1.220 capture replicated by CLIProxyAPI and have not been verified against a real capture yet; use "Probe TLS fingerprint" in the key editor to see the actual JA3/JA4.',
  '敏感词混淆': 'Sensitive word obfuscation',
  '每行一个词，不区分大小写；命中的词在第一个字符后插入零宽字符再发往上游。只处理系统提示与对话文本，不碰工具调用与结果。': 'One word per line, case-insensitive. Matched words get a zero-width character inserted after their first character before being sent upstream. Only system prompts and conversation text are processed; tool calls and results are left untouched.',
  '已配置': 'Configured',
  '词表变更会使提示词缓存失效': 'Changing the word list invalidates the prompt cache',
  '敏感词不能包含零宽字符': 'Sensitive words must not contain zero-width characters',
  '以下敏感词过短，每个词至少 2 个字符': 'The following sensitive words are too short; each word needs at least 2 characters',
  '以下敏感词过长，每个词最多 256 个字符': 'The sensitive words are too long; each word allows at most 256 characters',
  '敏感词最多 256 条': 'At most 256 sensitive words are allowed',
  '敏感词词表已变更，提示词缓存将失效': 'The sensitive word list changed; the prompt cache will be invalidated',
  '限流窗口': 'Rate limit windows',
  '重新物化最近一次响应头采集的窗口（不发上游请求）': 'Re-materialize the windows collected from the latest response headers (no upstream request)',
  '5 小时窗口': '5-hour window',
  '7 天窗口': '7-day window',
  '7 天窗口（Fable）': '7-day window (Fable)',
  '该窗口已拒绝请求': 'This window has rejected requests',
  ...
  '正在申请设备码...': 'Requesting device code...',
  '在 xAI 授权页面输入以下设备码': 'Enter this device code on the xAI authorization page',
  '复制设备码': 'Copy device code',
  '使用 xAI 账号登录 Grok Build，无需额外配置。授权后网关将通过 cli-chat-proxy 调用 Grok 模型。': 'Sign in with your xAI account for Grok Build. No extra setup needed; after authorization the gateway calls Grok models through cli-chat-proxy.',
  '粘贴 xAI Refresh Token，或 Grok CLI 导出的包含 refresh_token 的 JSON': 'Paste an xAI refresh token, or JSON exported by Grok CLI that contains refresh_token',
  'Grok Build 请使用设备授权或导入 Refresh Token。': 'Grok Build uses device authorization or refresh token import.',
  '设备授权仅支持 Kiro / Windsurf / Grok Build provider': 'Device authorization is only supported for Kiro / Windsurf / Grok Build providers',
  '前往登录': 'Go to sign-in',
  ...
  '上游支持 count_tokens（点击标记为不支持）': 'Upstream supports count_tokens (click to mark as unsupported)',
  '上游不支持 count_tokens，该端点不会接收 token 计数请求（点击标记为支持）': 'Upstream does not support count_tokens; this endpoint will not receive token counting requests (click to mark as supported)',
  '已标记为支持 count_tokens': 'Marked as supporting count_tokens',
  '已标记为不支持 count_tokens': 'Marked as not supporting count_tokens',
  ...
  '用户日志显示 Provider': 'Show provider in user logs',
...
const legacyPhraseEnglishMessages: Array<[string, string]> = [
  ...
  ['留空则沿用调度策略的默认值（默认 0，失败后直接转移）；设置后对该提供商的每个 Key 生效，1 表示失败后再试一次，共请求两次。', 'Leave empty to follow the routing policy default (0, fail over immediately); once set it applies to every key of this provider, and 1 retries once for two requests in total.'],
  ...
  ['控制注册、日志透明度与网关兼容行为', 'Control registration, log transparency, and gateway compatibility'],
  ['允许新用户自助注册账户', 'Allow new users to self-register accounts'],
  ['允许普通用户在自己的使用记录中查看最终实际使用的 Provider', 'Allow users to see the final provider used in their own usage records'],
